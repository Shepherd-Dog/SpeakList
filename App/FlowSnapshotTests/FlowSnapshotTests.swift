			...
			Async<UIImage> { callback in
				...
				let image = UIGraphicsImageRenderer(bounds: window.bounds).image {
					...
					window.drawHierarchy(
						in: window.bounds, afterScreenUpdates: true)
				}
...
class FlowSnapshotTests: XCTestCase {
	let xcodeCloudPath: StaticString =
		"/Volumes/workspace/repository/ci_scripts/FlowSnapshotTests.swift"
	let localPath: StaticString =
		"/Users/davidbrunow/Developer/SpeakList/App/SnapshotTests/FlowSnapshotTests.swift"
	...
	func testHappyPath() async throws {
		...
		let flowActions: [FlowRunner.NamedAction<AppFeature.Action>] = [
			...
			.init(
				name: "Edit Item Name",
				action: .listFeature(
					.addItem(.presented(.didEditItemName("Juicy Fruit"))))
			),
			.init(
				name: "Save New Item",
