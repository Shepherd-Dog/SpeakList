...
extension Snapshotting where Value == UIView, Format == UIImage {
	...
	public static func impreciseAccessibilityImage(
		showActivationPoints activationPointDisplayMode: ActivationPointDisplayMode =
			.whenOverridden,
		...
		return Snapshotting<UIView, UIImage>
			...
			.pullback { view in
				let containerView = AccessibilitySnapshotView(
					containedView: view,
					viewRenderingMode: drawHierarchyInKeyWindow
						? .drawHierarchyInRect : .renderLayerInContext,
				...
				if drawHierarchyInKeyWindow {
					if let keyWindow = UIApplication.shared.firstKeyWindow {
						...
						fatalError(
							"Unexpectedly could not find the key window"
						)
					}
				} else {
	...
	public static func impreciseImageWithSmartInvert(precision: Float) -> Snapshotting {
		...
		return Snapshotting<UIImage, UIImage>.image.pullback { view in
			...
			let image = renderer.image { context in
				view.drawHierarchyWithInvertedColors(
					in: view.bounds, using: context)
			}
