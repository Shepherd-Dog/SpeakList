import AVSpeechSynthesizerClient
	...
	public var body: some ReducerOf<Self> {
		Reduce { state, action in
			...
			case let .didReceiveSpeechRecognition(speech):
				guard
					let item = state.trip.allItems.first(where: {
						$0.checked == false
					})
				else {
					// TODO: Error
				...
				if speech.lowercased().contains("check") {
					state.trip.groups[
						id: item.preferredStoreLocation.location.name]?
						.items[id: item.id]?
						.checked = true
			...
			case let .didTapItem(item):
				state.trip.groups[id: item.preferredStoreLocation.location.name]?
					.items[id: item.id]?
					.checked.toggle()
