	...
	var body: some View {
		VStack {
			VStack {
				TextField(
					"Name",
					text: self.$store.groceryStore.name.sending(
						\.didEditStoreName)
				)
			}
			.padding(20)
			List {
				Section(
					header: Text("Layout"),
					footer: Text(
						"The layout of the store determines the order for items to be sorted."
					)
					.font(.caption)
				) {
					ForEach(self.store.groceryStore.locationsOrder) {
						location in
						Text(location.name)
					}
