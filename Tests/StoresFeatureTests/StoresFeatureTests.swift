...
class StoresFeatureTests: XCTestCase {
  func testBasics() async {
    ...
    await store.receive(
      .didReceiveGroceryStores(.init(uniqueElements: [
    ...
    await store.send(.addStoreButtonTapped) {
      $0.addStore = StoreFormFeature.State(groceryStore: GroceryStore(id: UUID(0), name: ""))
    }

    await store.send(.addStore(.presented(.didEditStoreName("Albertsons")))) {
      $0.addStore = StoreFormFeature.State(groceryStore: GroceryStore(id: UUID(0), name: "Albertsons"))
    }
    ...
    await store.send(.addStoreButtonTapped) {
      $0.addStore = StoreFormFeature.State(groceryStore: GroceryStore(id: UUID(1), name: ""))
    }
